	...
	Weather.prototype = {
		...
		init : function(config){
			...
			var weatherInfo = this.opts.weatherData || {},
				...
				templeteHtml = '<div class="weather-info"><h2>{#city#}<span>{#date#}</span></h2><div class="weather-show"><div class="day_weather"><span class="weather">{#day#} : {#day_weather#}</span>/<span class="temp">{#day_temp#}{#temp#}</span></div><div class="night_weather"><span class="weather">{#night#} : {#night_weather#}</span>/<span class="temp">{#night_temp#}{#temp#}</span></div></div><div class="weather-copy"><a href="http://julying.com/lab/weather/" target="_blank" title="&copy;{#wangzimo#}">julying</a></div></div>',
				i = 0
			;
			for( ; i < skyState_child_length ; i++){
				html.push('<div class="weather"><div class="'+ this.opts.sky +' sky sky-'+i+'"><div class="child-'+i+' child">');			
				if( weatherInfo.city ){
			...
			html = null ;	
			this.$weatherArea = this.$element.find('div.weather');	
