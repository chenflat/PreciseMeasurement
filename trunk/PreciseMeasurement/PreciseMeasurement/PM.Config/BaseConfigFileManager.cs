        ...
        public new static string ConfigFilePath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    if (context != null)
                    {
                        filename = context.Server.MapPath("~/PM.config");
                    ...
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "PM.config");
                    }

                    if (!File.Exists(filename))
                    {
                        throw new PMException("发生错误: 虚拟目录或网站根目录下没有正确的PM.config文件");
                    }
                }
                return filename;
            }
