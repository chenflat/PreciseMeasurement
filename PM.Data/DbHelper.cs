                ...
                if (m_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("PM.Data.{0}Provider, PM.Data.{0}", BaseConfigs.GetDbType), false, true));
                            }
                            catch
                            {
